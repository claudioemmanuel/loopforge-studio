      ...
      if (extraction.files.length === 0) {
        ...
        if (consecutiveNoFileIterations > maxNoFileRetries) {
          ...
          if (recoveryOrchestrator && useRecovery) {
            ...
            if (options.userId && options.taskId && options.taskTitle && options.repoName) {
            ...
            if (recoveryResult.success) {
              ...
              if (options.userId && options.taskId && options.taskTitle && options.repoName) {
                ...
                await publishRecoveryEvent(options.userId, recoverySuccessEvent);
              }
              ...
              if (options.userId && options.taskId && options.taskTitle && options.repoName) {
                const recoveryFailedEvent = createRecoveryEvent(
                  ...
                  4,
                  new Date(),
                  { statusText: "Recovery failed", error: recoveryResult.message }
                );
