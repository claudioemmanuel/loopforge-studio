/**
 * RevertExecutionSlot Use Case
 * Reverts task from executing back to ready (on execution failure)
 */
...
export interface RevertExecutionSlotInput {
  taskId: string;
}

export interface RevertExecutionSlotOutput {
  id: string;
  status: string;
  processingPhase: string | null;
}

export class RevertExecutionSlotUseCase {
  constructor(
  ...
  async execute(
    input: RevertExecutionSlotInput,
  ): Promise<Result<RevertExecutionSlotOutput, UseCaseError>> {
    // 1. Fetch task
    ...
    // 2. Verify task is in executing phase
    const state = task.getState();
    if (state.status !== "executing") {
      return Result.fail(
        new BusinessRuleError(
          "INVALID_PHASE",
          "Task must be in executing phase to revert",
        ),
      );
    }
    ...
    try {
      ...
      const output: RevertExecutionSlotOutput = {
        id: updatedState.id,
