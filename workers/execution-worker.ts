...
import { createPendingChanges } from "../lib/contexts/execution/infrastructure/pending-changes-repository";
import {
  createTestRun,
  completeTestRun,
} from "../lib/contexts/execution/infrastructure/test-run-repository";
...
async function runPreFlightChecks(
  ...
  const repo = await repositoryService.getById(repoId);
  ...
  if (repo.indexingStatus === "indexed") {
    repoIndexData =
      (await repositoryService.getRepoIndexByRepoId(repoId)) || null;
...
async function handleCascadingFailure(
  ...
  try {
    ...
    const dependentTasks =
      await taskService.listDependentsByBlocker(failedTaskId);
...
async function handleTaskOrchestration(
  ...
  try {
    ...
    const dependentTasks =
      await taskService.listDependentsByBlockerWithRepo(completedTaskId);
    ...
    for (const dep of dependentTasks) {
      ...
      const allBlockers = await taskService.listBlockersForTask(task.id);
      ...
      if (
        ...
        try {
          const completedBlocker =
            await taskService.getTaskWithLatestExecution(completedTaskId);
          if (completedBlocker) {
...
async function processExecution(
  ...
  const billingCheck = await billingService.checkLimits(userId);
  if (!billingCheck.withinLimits) {
    workerLogger.warn(
      { userId, usage: billingCheck.usage },
  ...
  if (skillsEnabled) {
    try {
      ...
      workerLogger.warn({ error }, "Skills framework not available");
    }
  }
        ...
        if (autoCreatePR && testsPassed !== false && fileChanges.length > 0) {
          ...
          try {
            const { getPendingChangesByTask } =
              await import("../lib/contexts/execution/infrastructure/pending-changes-repository");
            const { createExecutionCommit } =
              await import("../lib/contexts/execution/infrastructure/commit-repository");
            ...
            const { getLatestTestRun } =
              await import("../lib/contexts/execution/infrastructure/test-run-repository");
          ...
          try {
            ...
            if (pendingChanges.length > 0) {
              ...
              if (githubToken) {
                ...
                // Trigger downstream tasks when this task is fully completed.
                await handleTaskOrchestration(taskId, userId);
...
async function processBrainstorm(
  ...
  try {
    ...
    // Decrypt API key on demand from user record (never stored in Redis job data)
    const brainstormUser = await userService.getUserFull(userId);
    if (!brainstormUser) {
      throw new Error("User not found");
    }
...
async function processPlan(
  ...
  try {
    ...
    // Decrypt API key on demand from user record (never stored in Redis job data)
    const planUser = await userService.getUserFull(userId);
    if (!planUser) {
      throw new Error("User not found");
    }
