-- Kanban Enhancements: Activity Feed, Daily Summaries, Task Dependencies
-- Migration 0023

-- Activity event category enum
DO $$ BEGIN
...
-- Add task dependency fields to tasks table
...
-- Activity Events table (enhanced activity feed)
...
CREATE INDEX IF NOT EXISTS "activity_events_category_idx" ON "activity_events" ("event_category");

CREATE INDEX IF NOT EXISTS "activity_summaries_user_id_idx" ON "activity_summaries" ("user_id");
CREATE INDEX IF NOT EXISTS "activity_summaries_repo_id_idx" ON "activity_summaries" ("repo_id");
...
-- Index for finding blocked tasks efficiently
CREATE INDEX IF NOT EXISTS "tasks_blocked_by_ids_idx" ON "tasks" USING GIN ("blocked_by_ids");
