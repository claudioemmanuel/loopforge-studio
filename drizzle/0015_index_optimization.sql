...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_repo_id ON tasks(repo_id);

-- executions.task_id: Worker detail views, history lookups
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_executions_task_id ON executions(task_id);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_executions_task_created_desc
ON executions(task_id, created_at DESC);

-- worker_jobs: Worker history lookups filter by task and status
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_worker_jobs_task_status
ON worker_jobs(task_id, status);

-- worker_jobs: Query by job_id for status updates
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_worker_jobs_job_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_tasks_active_status
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_usage_records_user_period
ON usage_records(user_id, period_start);

-- worker_jobs: Analytics by created date
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_worker_jobs_created
