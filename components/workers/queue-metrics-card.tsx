...
export function QueueMetricsCard({ queues }: QueueMetricsCardProps) {
  ...
  const getTotalActive = () => {
    return (
      queues.brainstorm.active + queues.plan.active + queues.execution.active
    );
  };

  const getTotalWaiting = () => {
    return (
      queues.brainstorm.waiting + queues.plan.waiting + queues.execution.waiting
    );
  };
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <p
                className={`text-2xl font-bold tabular-nums ${getStatusColor(getTotalWaiting(), getTotalActive())}`}
              >
                {getTotalWaiting()}
              </p>
            </div>
